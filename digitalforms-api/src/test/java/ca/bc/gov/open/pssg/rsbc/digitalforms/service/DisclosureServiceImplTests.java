...
class DisclosureServiceImplTests {
	...
	public void init() {
		...
		when(disclosureService.setDisclosureSent(eq(IRP_TEST_NOTICE_NUMBER_SUCCESS), any(), any(), any(),
				eq(CORRELATION_ID)))
						.thenReturn(DisclosureResponse.successResponse("updatedTime", SUCCESS_CODE, SUCCESS_STATUS));
		when(disclosureService.setDisclosureSent(eq(IRP_TEST_NOTICE_NUMBER_ERROR), any(), any(), any(),
				eq(CORRELATION_ID))).thenReturn(DisclosureResponse.errorResponse(ERROR_STATUS));
	...
	void setDisclosureSentSuccess() throws DigitalFormsException {
		DisclosureResponse resp = serviceImpl.setDisclosureSent(IRP_TEST_NOTICE_NUMBER_SUCCESS, CORRELATION_ID,
				DISCLOSURE);
		Assertions.assertEquals(0, resp.getRespCode());
	}
	...
	void setDisclosureSentError() throws DigitalFormsException {
		DisclosureResponse resp = serviceImpl.setDisclosureSent(IRP_TEST_NOTICE_NUMBER_ERROR, CORRELATION_ID,
				DISCLOSURE);
		Assertions.assertEquals(-1, resp.getRespCode());
		Assertions.assertEquals(ERROR_STATUS, resp.getRespMsg());
	}
